...
const EditPetModal = ({ open, onClose, petId }) => {
  ...
  const [images, setImages] = useState([]); // Para almacenar las imágenes actuales
  ...
  useEffect(() => {
    ...
    if (open) {
      ...
      // Obtener la información de la mascota y las imágenes
      const fetchPetData = async () => {
        try {
          const petResponse = await axios.get(`${process.env.REACT_APP_API_URL}/pet/${petId}`, {
          ...
          const imagesResponse = await axios.get(`${process.env.REACT_APP_API_URL}/image/${petId}`, {
            headers: { Authorization: `Bearer ${authToken}` },
          });
          setImages(imagesResponse.data.map(img => `${process.env.REACT_APP_API_URL}/image/${petId}`));
